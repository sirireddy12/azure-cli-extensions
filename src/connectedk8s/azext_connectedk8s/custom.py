...
from azext_connectedk8s._client_factory import cf_storage
...
def troubleshoot(cmd, client, resource_group_name, cluster_name, kube_config=None, kube_context=None, location=None, storage_account=None,
    ...
    try:
        ...
        collect_logs(cmd, client, output_file, resource_group_name, cluster_name, storage_account, sas_token, kube_context, kube_config)
...
def collect_logs(cmd, client, output_file, resource_group_name, name, storage_account=None,
                sas_token=None,
                container_logs=None,
                kube_objects=None,
                node_logs=None,
    ...
    if storage_account is None:
        ...
        if storage_account_id is None:
            print("No storage account id specified. You can find the logs in this path on your local machine: ")
            clean_up_resources_and_deploy_periscope(temp_yaml_path, temp_yaml_file, deployment_yaml, kube_config)
            print()
...
def clean_up_resources_and_deploy_periscope(temp_yaml_path, temp_yaml_file, deployment_yaml, kube_config):
    try:
        ...
        try:
            ...
            subprocess_cmd = ["kubectl", "apply", "-f", temp_yaml_path, "-n", "aks-periscope", "--validate=false"]
            if kube_config:
                subprocess_cmd.extend(["--kubeconfig", kube_config])
